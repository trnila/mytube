...
prepare:function(c){var b=c.nette;if(b&&b.form){var d=b.e,e=c.data||{},g={};if(b.isSubmit)g[b.el.attr("name")]=b.el.val()||"";else if(b.isImage){var i=b.el.offset(),k=b.el.attr("name"),d=[Math.max(0,d.pageX-i.left),Math.max(0,d.pageY-i.top)];-1!==k.indexOf("[",0)?g[k]=d:(g[k+".x"]=d[0],g[k+".y"]=d[1])}"string"!=typeof e&&(e=a.param(e));g=a.param(g);c.data=b.form.serialize()+(g?"&"+g:"")+"&"+e}}});a.nette.ext("snippets",{success:function(a){if(a.snippets)for(var b in a.snippets)this.updateSnippet(b,
...
c.height=255;b=c.getContext("2d");f=b.createLinearGradient(0,0,0,c.height);f.addColorStop(0,"red");f.addColorStop(0.5,"yellow");f.addColorStop(1,"green");b.fillStyle=f;a()}},!1);var d})();$.editable.types.text.element=function(a){var e=$('<input type="text" />');"none"!=a.width&&e.attr("width",a.width);"none"!=a.height&&e.attr("height",a.height);e.attr("autocomplete","off");$(this).append(e);return e};
function editable(a,e,c){c=$.extend({indicator:"Ukl\u00e1dam...",tooltip:"Klikni pro \u00fapravu",ajaxoptions:{dataType:"json"},callback:function(b){if(b.error){var c=a.attr("title");a.attr("title",null);a.popover("destroy");a.popover({animation:!0,title:"Chyba",placement:"bottom",content:b.error});a.html(b.original);a.click();a.attr("title",c)}else a.popover("destroy"),a.html(b.value)}},c);a.editable(e,c)}
$(document).ready(function(){$(".editable").each(function(){var a=$(this),e="undefined"!=typeof a.data("editable-buttons");editable(a,a.closest("[data-editable-action]").attr("data-editable-action"),{type:a.data("editable-type")||"text",data:a.data("editable-data"),name:a.data("editable-name"),submitdata:{id:a.closest("[data-editable-id]").data("editable-id")},submit:e?'<button type="submit" class="btn btn-success">Ulo\u017eit</button>':null,cancel:e?'<button class="btn btn-danger">Zru\u0161it</button>':
null})})});$(function(){$.nette.ext("history").cache=!1;$.nette.init()});$(function(){var a;$("#video-search input").keyup(function(e){var c=$(this);clearInterval(a);a=setTimeout(function(){c.addClass("loading");$.nette.ajax({},c.closest("form"),e).done(function(){c.removeClass("loading")})},100)})});
$(document).ready(function(){$("[data-map-location]").each(function(){var a=$(this),e=new google.maps.Geocoder,c=new google.maps.Map(this,{zoom:8,mapTypeId:google.maps.MapTypeId.ROADMAP});e.geocode({address:a.attr("data-map-location")},function(b,d){d==google.maps.GeocoderStatus.OK?(c.setCenter(b[0].geometry.location),new google.maps.Marker({map:c,position:b[0].geometry.location,icon:a.attr("data-map-marker")?a.attr("data-map-marker"):null})):a.fadeOut()})})});
$(document).on("click","[data-confirm]",function(){return confirm($(this).data("confirm"))});
